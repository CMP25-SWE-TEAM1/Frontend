...
const ProfilePageEdit = (props) => {
  ...
  function EditProfile(e) {
    ...
    if (name !== user.nickname || bio !== user.bio || location !== user.location || website !== user.website || JSON.stringify(`${month}-${day}-${year}`) !== JSON.stringify(user.birthDate.slice(0, 10).split("-"))) {
      axios
        .patch(
    ...
    if (coverpage === DefaultCoverPage && DefaultCoverPage !== user.banner_image) {
      //delete the image
      axios
        .delete("https://backend.gigachat.cloudns.org/api/user/profile/banner", {
          headers: {
            authorization: "Bearer " + token,
          },
        })
        ...
        >
          <Box style={modalStyle} className={` ${darkMode ? `bg-black` : `bg-white`}`}>
            <div id="ParentDiv-test">
              <div id="ParentDiv2-test" className={`pop-up ${openCrop || opencoverCrop ? "!hidden" : ""}  no-scrollbar flex flex-col overflow-y-auto  `}>
                ...
                <form
                  id="EditProfileForm-test"
                  className="relative flex h-[60%] flex-col"
                  onSubmit={(e) => {
                    EditProfile(e)
                  }}
                >
                  ...
                  <EditDate selectdatedisplay={selectdatedisplay} setSelectdatedisplay={setSelectdatedisplay} htmldate={`${month} ${day}, ${year}`} month={month} setMonth={setMonth} day={day} setDay={setDay} year={year} setYear={setYear}></EditDate>
                </form>
              </div>
