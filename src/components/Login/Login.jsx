...
const Login = ({ openModal, handleCloseModal, setLocation }) => {
  ...
  const APIs = {
    mock: { emailExistAPI: "httpss://ca224727-23e8-4fb6-b73e-dc8eac260c2d.mock.pstmn.io/checkEmail" },
    actual: {
          ...
          <div className="pop-up min-w-[350px] bg-white dark:bg-black md:rounded-2xl " id="mahmoud_login_box">
            ...
            <div id="firstPage" className="m-auto w-[300px]">
              <div>
                ...
                <button type="button" id="next" className="btn mt-2 bg-black dark:bg-white" onClick={handleEmailCheck} disabled={userName === ""}>
                  Next
                </button>
                <Link
                  onClick={() => {
                ...
                <span className="mt-5 text-slate-400">
                  ...
                  <Link to={"/Signup"} className={`text-${getColor(themeColor)}`}>
                    Sign Up
                  </Link>{" "}
                </span>
            ...
            <div id="secondPage" className="m-auto hidden  max-w-[300px]">
              <div>
                ...
                <form action="/" method="post" className="flex flex-col gap-5" autoComplete="off" onSubmit={handleLoginEvent} id="mahmoud_form">
                  ...
                  <button id="login" type="submit" className="btn -mb-4 mt-36 h-14 rounded-3xl bg-black dark:bg-white" disabled={password === ""}>
